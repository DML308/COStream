...
struct TComPicYuv
{
	Pel*  m_apiPicBufY[3];           ///< Buffer (including margin)
	Pel*  m_apiPicBufU;
	Pel*  m_apiPicBufV;
...
{
	stream<unsigned long ul_m_statData,unsigned long ul_srcYuv,unsigned long ul_pPic,unsigned long ul_m_pcEncTop>Out1,Out2;
...
composite Main()
{
	...
	//stream<unsigned long ul_m_statData,unsigned long ul_srcYuv,unsigned long ul_pPic,unsigned long ul_m_pcEncTop>pDst,pDst1; 
    //stream<unsigned long ul_reconParams,unsigned long ul_srcYuv,unsigned long ul_pPic,unsigned long ul_m_pcEncTop> Out1;
	//stream<unsigned long ul_reconParams,unsigned long ul_pPic,unsigned long ul_m_pcEncTop> Out2;
	//stream<unsigned long ul_pPic,unsigned long ul_m_pcEncTop>Out;
	
	pSrc = Source()  //Source() 没有问题
	{
		work
		{
	...
	//pDst1 = BordedExtension(pSrc)();
	//pDst =  getStatistics(pDst1)();
	//Out1 = decideModeRDO_recBlkSAOParam(pDst)();
	//Out2 = offsetCTU(Out1)();
	//Out = numLcusForSAO(Out2)();
	pDst =  SAOProcess(pSrc)();
	
	/*pDst = xx(pSrc)
	{
		work
		{
			pDst[0].ul_pPic = pSrc[0].ul_pPic;	
			pDst[0].ul_m_pcEncTop = pSrc[0].ul_m_pcEncTop;	
		}
		window
		{
			pSrc sliding(1,1);
