...
void MAFLPartition::SssgPartition(SchedulerSSG *sssg,int level)
{
	assert(level==1);
	...
	for (iter_1=flatNodes_.begin();iter_1!=flatNodes_.end();++iter_1)//遍历所有结点
	{
		...
		{
			FlatNode2PartitionNum.insert(make_pair((*iter_1),0));//建立节点到划分编号的映射
		...
		{
			FlatNode2PartitionNum.insert(make_pair((*iter_1),GpuNum));//建立节点到划分编号的映射
...
void MAFLPartition::SetMultiNum2FlatNode()
{
	vector<FlatNode *>::iterator iter_1;
	for (iter_1=flatNodes_.begin();iter_1!=flatNodes_.end();++iter_1)//遍历所有结点
	{
...
int MAFLPartition::UporDownStatelessNode(FlatNode *node)
{
...
vector<FlatNode *> MAFLPartition::FindNodeInOneDevice(int parts)
{
...
int MAFLPartition::Index2Device(int index)
{
...
void MAFLPartition::SetGpuNum(SchedulerSSG *sssg)
{
