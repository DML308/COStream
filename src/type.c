...
/***********************--------------Define For SPL----------****************************/
  case T_COMMAL_DECL: strcat(array, "commal_decl "); break;
  case T_STREAM_DECL: strcat(array, "stream_decl "); break;
/***********************--------------Define For SPL----------****************************/  
  case 0: break;
  default: UNREACHABLE;
  }
...
GLOBAL TypeQual NodeTq(Node *node)
{
  ...
  case Fdcl:
	...
	/***********************--------------Define For SPL----------****************************/
  case STRdcl:
	  return node->u.strdcl.tq;
  case Comdcl:
	  return node->u.comdcl.tq;
  case Operdcl:
	  return node->u.operdcl.tq;
...
GLOBAL void NodeSetTq(Node *node, TypeQual mask, TypeQual tq)
{
  ...
  case Fdcl:
	...
	/***********************--------------Define For SPL----------****************************/
  case Comdcl:
	  node->u.comdcl.tq &= ~mask;
	  node->u.comdcl.tq |= tq;
	  break;
  case Operdcl:
	  node->u.operdcl.tq &= ~mask;
	  node->u.operdcl.tq |= tq;
	  break;
...
GLOBAL TypeQual NodeDeclLocation(Node *node)
{
	...
	assert(node->typ == Decl || node->typ == Binop);  //cz,添加Binop的判断
...
PRIVATE Node *lookup_identifier(Node *id)
{
	...
	var = Undeclared;
	SyntaxErrorCoord(id->coord, "error: undeclared variable `%s'", name);
    } else {
	REFERENCE(var);
	if (TrackIds) {
...
{ 
	...
	if (var->u.decl.references == 0 && type->typ != Fdcl && type->typ != Comdcl)
	{
		/* give warning only if unused local variable */
		if (NodeDeclLocation(var) == T_BLOCK_DECL && NodeStorageClass(var) != T_EXTERN)
...
{ Node *type;
  ...
  case STRdcl:
	  if (!IsStreamComplete(type)) {
		  SyntaxErrorCoord(node->coord, "Can't compute size of incomplete stream type");
		  return 1;
	  }
	  else return Stream_Sizeof(type->u.strdcl.type);
...
/***********************--------------Define For SPL----------****************************/
GLOBAL Bool IsStreamComplete(Node *node)
{ 
	assert(node);
	assert(node->typ == STRdcl);
	return node->u.strdcl.type->complete;
}
/***********************--------------Define For SPL----------****************************/
...
/* IsPrimitiveStmt() is true_ for expressions and statements which
...
GLOBAL Bool TypeEqualQualified(Node *node1, Node *node2, Bool strict_toplevel, Bool strict_recursive)
{
  ...
  case STRdcl:
	  ...
	  /* Check the tags also in case we are still just scanning */
	  return SUE_SameTagp(node1->u.strdcl.type, node2->u.strdcl.type);
  case Tdef:
