...
void HClusterPGTDPartition::SssgHClusterPartition()
{
	/*进入进程级划分*/
	processPartition();
...
/*进程级划分*/
void HClusterPGTDPartition::processPartition()
{
...
/*线程级划分*/
void HClusterPGTDPartition::threadPartition()
{
	/*对每个集群节点的进程级划分结果*/
	for (int i = 0; i < m_nClusters; ++i)
	{
		...
		if (isGPUClusterNode(i))
		{
			...
			m_htds[i].first = new HClusterThreadDynamicPartition(nCore, m_sssg, m_statefulNodes[i]);
			
			/*进入CPU端线程级划分*/
			map<FlatNode *, int> tmpFlatNode2CoreNum;
			map<int, vector<FlatNode*>> tmpCoreNum2FlatNodes = m_htds[i].first->threadPartition();
			this->m_coreNum2FlatNodes[i] = tmpCoreNum2FlatNodes;
			m_allComm += m_htds[i].first->m_allComm;
			...
			/*GPU端二级划分*/
			m_htds[i].second = new HClusterThreadDynamicPartition(nGpu, m_sssg, m_statelessNodes[i]);
			
			map<FlatNode *, int> tmpFlatNode2GpuNum;
			map<int, vector<FlatNode*>> tmpGpuNum2FlatNodes = m_htds[i].second->threadPartition();
			this->m_gpuNum2StatelessNodes[i] = tmpGpuNum2FlatNodes;
			m_allComm += m_htds[i].second->m_allComm;
			...
			nCore = this->m_hCluster2CpuAndGpu[i].first;
			m_htds[i].first = new HClusterThreadDynamicPartition(nCore, m_sssg, m_hClusterNum2FlatNodes[i]);
			m_htds[i].second = NULL;
